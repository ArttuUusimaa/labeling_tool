...
WebApp.connectHandlers.use("/api/pcdtextlabel", generateLabelOutput.bind({fileMode: false}));
WebApp.connectHandlers.use("/api/pcdfilelabel", generateLabelOutput.bind({fileMode: true}));
WebApp.connectHandlers.use("/api/pcdtextobject", generateObjectOutput.bind({fileMode: false}));
WebApp.connectHandlers.use("/api/pcdfileobject", generateObjectOutput.bind({fileMode: true}));
...
/* Here I modified the function generatePCDOutput to print only either the labels or the objects
   rather than the contents of the entire PCD file. */
...
function generateLabelOutput (req, res, next) {
    const pcdFile = imagesFolder + decodeURIComponent(req.url);
    ...
    readFile(pcdFile, (err, content) => {
        ...
        readFile(labelFile, (labelErr, labelContent) => {
            if (labelErr) {
                res.end("Error while pasing labels file.")
            }
            const labels = SseDataWorkerServer.uncompress(labelContent);
...
function generateObjectOutput (req, res, next) {
    const pcdFile = imagesFolder + decodeURIComponent(req.url);
    ...
    readFile(pcdFile, (err, content) => {
        ...
        readFile(labelFile, (labelErr, labelContent) => {
            if (labelErr) {
                res.end("Error parsing labels file.")
            }
            const labels = SseDataWorkerServer.uncompress(labelContent);
